...
function parseFilter(filter: string): FilterAst {
	...
	const parsePrimary = (): FilterAst => {
		...
		if (!operatorToken || operatorToken.type !== "word") {
			throw new Error(`Invalid filter syntax near '${attribute}': operator expected`);
		}
		...
		if (!valueToken || (valueToken.type !== "word" && valueToken.type !== "string")) {
			throw new Error(`Invalid filter syntax near '${attribute} ${operator}': value expected`);
		}
	...
	const parseAndExpression = (): FilterAst => {
		...
		while (true) {
			const token = current();
			if (!token || token.type !== "word" || token.value.toLowerCase() !== "and") {
				break;
			}
	...
	const parseOrExpression = (): FilterAst => {
		...
		while (true) {
			const token = current();
			if (!token || token.type !== "word" || token.value.toLowerCase() !== "or") {
				break;
			}
...
function assertAttributeAllowed(attribute: string, allowedAttributes: Set<string>): void {
	if (!allowedAttributes.has(attribute)) {
		throw new Error(`Invalid attribute '${attribute}' in filter/orderBy`);
	}
}

function toWhereClause(ast: FilterAst, allowedAttributes: Set<string>): WhereOptions {
	...
	const expressions = ast.operands.map((operand) => toWhereClause(operand, allowedAttributes));
	return {
		[ast.type === "and" ? Op.and : Op.or]: expressions,
	};
}

function parseOrderBy(orderBy: string | undefined, allowedAttributes: Set<string>): Order | undefined {
	if (!orderBy || orderBy.trim() === "") {
		return undefined;
	}

	const order: Order = [];
	const segments = orderBy.split(",").map((segment) => segment.trim()).filter(Boolean);
	...
	for (const segment of segments) {
		...
		if (direction !== "asc" && direction !== "desc") {
			throw new Error(`Invalid order direction '${directionRaw}' for '${attribute}'`);
		}
...
function mergeWhere(baseWhere: WhereOptions | undefined, filterWhere: WhereOptions | undefined): WhereOptions | undefined {
...
function buildPaginateQueryString(params: PaginateRequestParams, page: number, size: number): string {
...
function buildPaginateLinks(
	...
	const createHref = (page: number): string => `${base}${buildPaginateQueryString(params, page, meta.size)}`;
...
export function formatPaginateResponse<T>(
	...
	if (includeLinks) {
		response._links = buildPaginateLinks(params, { page, size, totalElements, totalPages }, meta?.baseUrl);
	}
...
export async function paginateModel<T extends Model>(
	model: ModelStatic<T>,
	params: PaginateRequestParams,
	baseOptions: Omit<FindAndCountOptions, "where" | "order" | "limit" | "offset"> & {
		where?: WhereOptions;
	} = {},
	...
	const filterWhere = params.filter && params.filter.trim() !== ""
