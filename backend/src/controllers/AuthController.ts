...
/**
 * Helper function to parse the user_id from an SFC email. Returns null if the email is not a valid SFC email.
 * @param value
 * @returns
 */
function parseSfcLoginEmail(value: string): number | null {
  const email = value.trim().toLowerCase();
  const [localPart, domain] = email.split("@");

  if (!localPart || !domain || domain !== "sfc.gov.my") {
    return null;
  }

  const user_id = parseInt(localPart, 10);
...
export const token = async (
  ...
  try {
    // Login is done using {username, password} due to legacy reasons. But in actuality the user is loggin in using their sfc email
    // which is derived from {user_id}@sfc.gov.my.
    const { username, password } = req.body as TokenRequestBody;
    const user_id = parseSfcLoginEmail(username ?? "");
    if (!user_id || !password) {
      res.status(401).json({ message: "Incorrect email or password" });
      return;
    }

    // Find the user by matching the user_id portion of the email (before @sfc.gov.my)
...
export const forgotPassword = async (
  ...
  try {
    ...
    const user_id = parseSfcLoginEmail(normalizedEmail);
    ...
    if (error instanceof DatabaseError) {
      ...
      res
        .status(500)
        .json({ message: "Database error during password reset request" });
      next();
      return;
    }
    next(error);
...
export const changePassword = async (
  ...
  try {
    ...
    if (error instanceof DatabaseError) {
      ...
      res
        .status(500)
        .json({ message: "Database error during password change" });
      next();
      return;
    }
    next(error);
