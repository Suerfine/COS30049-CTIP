...
const resolveBaseDate = (user: RegistrationFactoryUser, adminUser: RegistrationFactoryAdminUser): Date => {
  const candidates = [user.created_at, adminUser.created_at].filter((date): date is Date => date instanceof Date);
...
const buildRegistrationData = (): Omit<RegistrationFactoryAttributes, "id" | "user_id" | "reviewed_by_user_id" | "status" | "created_at" | "updated_at" | "deleted_at"> => {
  return {
...
export const buildRegistration = (
  ...
  const createdAt = registrationOverrides.created_at instanceof Date
...
export const buildRegistrationHistory = (
  ...
  const baseDate = resolveBaseDate(user, adminUser);
  const failedRegistrations = Array.from({ length: failedAttemptsCount }, (_, index) => {
