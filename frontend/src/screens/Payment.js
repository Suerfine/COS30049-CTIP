...
const Payment = ({ navigation, route }) => {
    const courseTitle = "Advanced Park Safety & First Aid";
    const amount = "RM 150.00";
    ...
    const handleUploadReceipt = () => {
        setUploading(true);
        setTimeout(() => {
            setReceipt('https://via.placeholder.com/300x400.png?text=Receipt+Preview');
            setUploading(false);
        }, 1500);
    };
    ...
    const handleConfirmPayment = () => {
        if (!receipt) {
            Alert.alert("Error", "Please upload your bank receipt first.");
            return;
        }
                ...
                >
                    {receipt ? (
                        ...
                        <View style={styles.uploadPlaceholder}>
                            ...
                            <Text style={styles.uploadText}>{uploading ? "Uploading..." : "Click to select file"}</Text>
                        </View>
                    )}
                </TouchableOpacity>
                {receipt && (
                    <TouchableOpacity onPress={() => setReceipt(null)}>
                        <Text style={styles.reselectText}>Choose a different file</Text>
                    </TouchableOpacity>
                )}
            </View>

            <TouchableOpacity 
                style={[styles.confirmBtn, !receipt && styles.btnDisabled]} 
                onPress={handleConfirmPayment}
            >
                <Text style={styles.confirmBtnText}>Confirm & Submit Payment</Text>
            </TouchableOpacity>
