...
import { View, Text, StyleSheet, ScrollView, TextInput, Button, Alert, TouchableOpacity, Modal, Image, useWindowDimensions } from 'react-native';
...
export default function DetectionScreen() {
  ...
  const [serverConfig, setServerConfig] = useState({ host: getAutoHost(), port: '8000' });
  const [tempConfig, setTempConfig] = useState({ host: getAutoHost(), port: '8000' });
  ...
  let COVER_SCALE = 1, OFFSET_X = 0, OFFSET_Y = 0;
  if (cameraLayout) {
  ...
  useEffect(() => {
    AsyncStorage.getItem(SERVER_CONFIG_STORAGE_KEY).then((saved) => {
      ...
      try {
        ...
        const loaded = { host: parsed?.host || getAutoHost(), port: parsed?.port || '8000' };
  ...
  useEffect(() => {
    AsyncStorage.setItem(SERVER_CONFIG_STORAGE_KEY, JSON.stringify(serverConfig)).catch(() => {});
  ...
  const fetchAnomalyEvents = async () => {
    ...
    try {
      const response = await apiClient.get(`/anomaly-events/${currentUser.id}?includeResolved=false`);
  ...
  const resolveEvent = async (eventId) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.message || 'Failed to resolve anomaly event.';
  ...
  const renderSkeletonLine = (kp1, kp2, index) => {
    ...
    const x1 = toRenderX(kp1.x), y1 = toRenderY(kp1.y);
    const x2 = toRenderX(kp2.x), y2 = toRenderY(kp2.y);
  ...
  const renderKeypoint = (kp, index) => {
    ...
    const x = toRenderX(kp.x), y = toRenderY(kp.y);
    const KP_RADIUS = 5;
    ...
    return (
      <View style={styles.configContainer}>
        <ScrollView style={styles.configFormScroll}>
          ...
          <TextInput style={styles.input} placeholder={getAutoHost()} value={tempConfig.host} onChangeText={(t) => setTempConfig({...tempConfig, host: t})} />
          ...
          <View style={styles.buttonContainer}><Button title="Save" onPress={() => { setServerConfig({ host: (tempConfig.host || '').trim() || getAutoHost(), port: (tempConfig.port || '').trim() || '8000' }); setConfigMode(false); }} color="#4CAF50" /></View>
  ...
  return (
    <View style={[styles.appContainer, isCompact && styles.appContainerCompact]}>
          ...
          <View style={styles.detailModal}>
            <ScrollView>
              <View style={styles.detailHeader}>
                <Text style={styles.detailTitle}>{selectedEvent ? getEventLabel(selectedEvent.event_type) : ''}</Text>
              ...
              {selectedEvent && (
                <>
                  <View style={styles.detailMetaGrid}>
                    <Text style={styles.detailMetaText}><Text style={styles.detailMetaLabel}>Detected: </Text>{new Date(selectedEvent.created_at).toLocaleString()}</Text>
                    <Text style={styles.detailMetaText}><Text style={styles.detailMetaLabel}>Confidence: </Text>{getEventConfidence(selectedEvent)}</Text>
                    <Text style={styles.detailMetaText}><Text style={styles.detailMetaLabel}>Latitude: </Text>{selectedEvent.latitude ?? 'N/A'}</Text>
                    <Text style={styles.detailMetaText}><Text style={styles.detailMetaLabel}>Longitude: </Text>{selectedEvent.longitude ?? 'N/A'}</Text>
                  </View>
                  {selectedEvent.annotated_frame_base64 ? (
                    <Image
                      source={{ uri: `data:image/jpeg;base64,${selectedEvent.annotated_frame_base64}` }}
                      style={styles.detailImage}
                      resizeMode="contain"
                    />
                  ) : (
                    <View style={styles.emptyEvidence}>
                      <Text style={styles.emptyEvidenceText}>No annotated frame recorded for this event.</Text>
                    </View>
                  )}
                  <ScrollView horizontal style={styles.metadataBlock}>
                    <Text style={styles.metadataText}>{JSON.stringify(selectedEvent.metadata || {}, null, 2)}</Text>
                  </ScrollView>
                  {!selectedEvent.is_resolved && (
                    <TouchableOpacity
                      style={[styles.resolveButton, resolvingEventId === selectedEvent.id && styles.resolveButtonDisabled]}
      ...
      <View style={[styles.mainContent, isCompact && styles.mainContentCompact]}>
        <View style={styles.titleBar}>
          <View style={styles.titleCopy}>
            ...
            <Text style={styles.pageSubtitle}>Monitor and review active anomaly alerts</Text>
          </View>
          <TouchableOpacity style={styles.refreshButtonContainer} onPress={fetchAnomalyEvents}>
          ...
          <View style={styles.eventsPanel}>
            <View style={styles.panelHeader}>
              ...
              <View style={styles.badgeCount}>
                <Text style={styles.badgeCountText}>{anomalyEvents.length}</Text>
              </View>
            </View>
                  ...
                  <TouchableOpacity key={event.id} onPress={() => setSelectedEvent(event)}>
                    <View style={styles.eventCard}>
                      <View style={styles.eventHeader}>
                        ...
                        <Text style={styles.eventConfidence}>{getEventConfidence(event)}</Text>
                      </View>
      ...
      {/* RIGHT PANEL: Camera */}
      <View style={[styles.cameraSidebar, isCompact && styles.cameraSidebarCompact]}>
        <View
          style={[styles.cameraWrapper, isCompact && styles.cameraWrapperCompact]}
          onLayout={(event) => setCameraLayout({ width: event.nativeEvent.layout.width, height: event.nativeEvent.layout.height })}
        >
          <CameraView ref={cameraRef} style={styles.camera} facing="back">

            <Text style={[styles.status, { backgroundColor: isConnected ? 'rgba(16, 185, 129, 0.85)' : 'rgba(220, 38, 38, 0.85)' }]}>
            ...
            {isConnected && cameraLayout && latestResult?.detections?.map((det, index) => {
              ...
              return (
                <View key={`det-${index}`} style={[styles.boundingBox, { left: toRenderX(x1), top: toRenderY(y1), width: (x2 - x1) * COVER_SCALE, height: (y2 - y1) * COVER_SCALE }]}>
                  <Text style={styles.detectionLabel}>{det.class_name} {Math.round(det.confidence * 100)}%</Text>
                </View>
              );
            })}
            ...
            {isConnected && cameraLayout && latestResult?.compliance?.hand_boxes?.map((box, index) => {
              ...
              return (
                <View key={`hand-${index}`} style={[styles.interactionBox, {
                  left: toRenderX(box[0]), top: toRenderY(box[1]), width: (box[2] - box[0]) * COVER_SCALE, height: (box[3] - box[1]) * COVER_SCALE,
            ...
            {latestResult?.compliance?.extended_touch_animal && <Text style={[styles.warningText, {backgroundColor: 'rgba(255, 165, 0, 0.8)'}]}>EXTENDED ANIMAL TOUCH</Text>}
            {latestResult?.compliance?.extended_touch_plant && <Text style={[styles.warningText, {backgroundColor: 'rgba(255, 165, 0, 0.8)'}]}>EXTENDED PLANT TOUCH</Text>}
            ...
            {/* Settings Button */}
            <Text style={styles.settingsButton} onPress={() => setConfigMode(true)}>Config</Text>
...
const styles = StyleSheet.create({
  ...
  mainContentCompact: { flex: 1, paddingHorizontal: 14, paddingTop: 16, paddingBottom: 10 },
  title: { fontSize: 22, fontWeight: 'bold', color: '#111827' },
  pageSubtitle: { marginTop: 4, color: '#6b7280', fontSize: 12 },
  titleCopy: { flex: 1, paddingRight: 10 },
  titleBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, gap: 10 },
  refreshButtonContainer: { flexShrink: 0, borderWidth: 1, borderColor: '#bbf7d0', borderRadius: 8, backgroundColor: '#ecfdf5', paddingHorizontal: 12, paddingVertical: 8 },
  refreshButton: { fontSize: 13, fontWeight: '600', color: '#0a6340' },
  loadingView: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f6f8f7' },

  eventsPanel: { flex: 1, backgroundColor: '#ffffff', borderRadius: 12, borderWidth: 1, borderColor: '#e5e7eb', padding: 12 },
  ...
  badgeCount: { backgroundColor: '#f3f4f6', borderRadius: 99, paddingHorizontal: 8, paddingVertical: 4 },
  badgeCountText: { fontSize: 12, fontWeight: '700', color: '#374151' },
  eventsList: { flex: 1 },
  eventCard: { backgroundColor: '#f9fafb', borderRadius: 10, padding: 12, marginBottom: 10, borderWidth: 1, borderColor: '#e5e7eb' },
  eventHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  eventMetaRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8 },
  eventType: { color: '#111827', fontWeight: '700', fontSize: 13, flex: 1, paddingRight: 8 },
  eventConfidence: { color: '#0f766e', fontWeight: '700', fontSize: 12 },
  eventTime: { color: '#6b7280', fontSize: 11, fontStyle: 'italic', flex: 1 },
  activePill: { backgroundColor: '#dcfce7', borderRadius: 999, paddingHorizontal: 7, paddingVertical: 2 },
  ...
  cameraSidebarCompact: { flex: 0, height: 220, backgroundColor: '#f6f8f7', paddingHorizontal: 14, paddingTop: 0, paddingBottom: 12 },
  ...
  status: { position: 'absolute', top: 10, left: 10, color: 'white', fontWeight: 'bold', fontSize: 12, padding: 6, borderRadius: 6 },
  warningText: { position: 'absolute', top: 100, alignSelf: 'center', color: '#fee2e2', fontSize: 13, fontWeight: 'bold', backgroundColor: 'rgba(127, 29, 29, 0.85)', padding: 8, borderRadius: 6 },
  statsPanel: { position: 'absolute', bottom: 10, right: 10, backgroundColor: 'rgba(17,24,39,0.8)', padding: 10, borderRadius: 8 },
  statText: { color: '#e5e7eb', fontSize: 10, marginVertical: 2 },
  boundingBox: { position: 'absolute', borderWidth: 2, borderColor: '#22c55e', backgroundColor: 'rgba(34, 197, 94, 0.15)' },
  detectionLabel: { position: 'absolute', top: 2, left: 2, color: '#dcfce7', backgroundColor: 'rgba(15,23,42,0.85)', paddingHorizontal: 4, paddingVertical: 2, fontSize: 11, fontWeight: 'bold', borderRadius: 3 },
  interactionBox: { position: 'absolute', borderWidth: 2, borderRadius: 3 },
  interactionLabel: { position: 'absolute', top: 2, left: 2, backgroundColor: 'rgba(0,0,0,0.75)', paddingHorizontal: 3, paddingVertical: 1, fontSize: 10, fontWeight: 'bold', borderRadius: 2 },
  settingsButton: { position: 'absolute', bottom: 10, left: 10, color: 'white', fontSize: 12, fontWeight: '700', backgroundColor: 'rgba(17,24,39,0.75)', paddingHorizontal: 10, paddingVertical: 7, borderRadius: 16, overflow: 'hidden' },

  detailModalOverlay: { flex: 1, backgroundColor: 'rgba(15, 23, 42, 0.6)', justifyContent: 'center', alignItems: 'center', padding: 16 },
  detailModal: { width: '95%', maxHeight: '85%', backgroundColor: 'white', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#e5e7eb' },
  detailHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  detailTitle: { fontSize: 17, fontWeight: '700', color: '#111827', flexShrink: 1 },
  detailClose: { color: '#374151', fontWeight: '600', fontSize: 14, borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8 },
  detailMetaGrid: { gap: 6, marginBottom: 12 },
  detailMetaText: { color: '#374151', fontSize: 13 },
  detailMetaLabel: { fontWeight: '700' },
  detailImage: { width: '100%', height: 200, borderRadius: 10, borderWidth: 1, borderColor: '#e5e7eb', marginBottom: 12, backgroundColor: '#111827' },
  emptyEvidence: { padding: 10, borderWidth: 1, borderColor: '#d1d5db', borderStyle: 'dashed', borderRadius: 8, marginBottom: 12 },
  emptyEvidenceText: { color: '#6b7280', fontSize: 13 },
  metadataBlock: { backgroundColor: '#f9fafb', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, padding: 10, maxHeight: 140, marginBottom: 12 },
  metadataText: { fontSize: 11, color: '#374151', fontFamily: 'monospace' },
  resolveButton: { marginTop: 4, backgroundColor: '#065f46', borderRadius: 8, padding: 12, alignItems: 'center' },
  ...
  configFormScroll: { padding: 20, backgroundColor: '#1e1e1e', borderRadius: 8 },
  configTitle: { fontSize: 22, fontWeight: 'bold', marginBottom: 20, color: 'white' },
  configLabel: { fontSize: 14, fontWeight: '600', marginTop: 15, marginBottom: 5, color: '#ccc' },
